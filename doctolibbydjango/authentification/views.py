...
def connexion(request):
    ...
    if request.method == "POST": 
        ...
        verification = authenticate(username = username, password = motDePasse)
...
def inscription(request):
    ...
    if request.method == "POST": 
        username = request.POST["username"]
        motDePasse = request.POST["motDePasse"]
        nouveauCompte = Utilisateur.objects.create_user(username = username, password = motDePasse)

        return redirect("connexion")

    return render(request, "inscription.html", {"ideeMDP" : ideeMDP.replace(" ", "")})
